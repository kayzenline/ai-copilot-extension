...
describe('Gemini API Service', () => {
    ...
    describe('callGeminiAPI', () => {
        ...
        it('should throw error when API key is missing', async () => {
            await expect(callGeminiAPI('sys prompt', 'user prompt', '', 'gemini-2.5-flash', '')).rejects.toThrow('API Key is required');
        });
        it('should send formatted request body and header to Gemini endpoint', async () => {
            ...
            const result = await callGeminiAPI('System instruction', 'User text', 'test-key-xyz', 'gemini-2.5-flash', 'https://generativelanguage.googleapis.com/v1beta/models');
            expect(result).toBe('Gemini explanation output');
            expect(fetch).toHaveBeenCalledWith('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent', expect.objectContaining({
                method: 'POST',
                headers: {
        ...
        it('should throw formatted error when API returns error status', async () => {
            ...
            await expect(callGeminiAPI('sys', 'user', 'bad-key', 'gemini-2.5-flash', '')).rejects.toThrow('Authentication failed (HTTP 403)');
        });
        it('should throw formatted error when model is not found (HTTP 404)', async () => {
            ...
            await expect(callGeminiAPI('sys', 'user', 'valid-key', 'non-existent-model', '')).rejects.toThrow('Model not found (HTTP 404)');
        });
    });
});
