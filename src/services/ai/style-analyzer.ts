import { StyleProfile } from '../../types/extension';

export class StyleAnalyzerService {
  static getAnalysisSystemPrompt(): string {
...
Respond in JSON format: {"tone": "...", "length": "...", "traits": ["...", "..."]}`;
  ...
  static parseAnalysisResponse(response: string, fallbackSample: string): StyleProfile {
  ...
  static buildGhostSuffix(profile: StyleProfile): string {
    if (!profile.traits.length && !profile.tone) return '';

    const parts = ['Write the response matching this specific writing style:'];
    if (profile.tone) parts.push(`Tone: ${profile.tone}`);
    if (profile.length) parts.push(`Length preference: ${profile.length}`);
    if (profile.traits.length) parts.push(`Traits: ${profile.traits.join(', ')}`);
