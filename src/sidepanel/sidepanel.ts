...
import { ExtensionSettings, StyleProfile, ExtensionMessage } from '../types/extension';
...
(() => {
  ...
  const els = {
    // Tabs
    tabBtns: $$<HTMLButtonElement>('.tab-btn'),
    tabPanels: $$<HTMLElement>('.tab-panel'),
    // Search
    searchInput: $<HTMLDivElement>('#search-input-area'),
    btnExplain: $<HTMLButtonElement>('#btn-explain'),
    searchResult: $<HTMLDivElement>('#search-result'),
    searchResultText: $<HTMLDivElement>('#search-result-text'),
    // Sandbox
    sandboxInput: $<HTMLTextAreaElement>('#sandbox-input'),
    sandboxOutput: $<HTMLDivElement>('#sandbox-output'),
    btnRefine: $<HTMLButtonElement>('#btn-refine'),
    // Email
    emailThread: $<HTMLDivElement>('#email-thread'),
    emailKeywords: $<HTMLTextAreaElement>('#email-keywords'),
    emailTone: $<HTMLSelectElement>('#email-tone'),
    emailLength: $<HTMLSelectElement>('#email-length'),
    btnDraft: $<HTMLButtonElement>('#btn-draft'),
    emailResult: $<HTMLDivElement>('#email-result'),
    emailResultText: $<HTMLDivElement>('#email-result-text'),
    btnInsertDraft: $<HTMLButtonElement>('#btn-insert-draft'),
    // Settings
    btnSettings: $<HTMLButtonElement>('#btn-settings'),
    settingsModal: $<HTMLDivElement>('#settings-modal'),
    btnCloseSettings: $<HTMLButtonElement>('#btn-close-settings'),
    btnSaveSettings: $<HTMLButtonElement>('#btn-save-settings'),
    btnTestConnection: $<HTMLButtonElement>('#btn-test-connection'),
    connectionStatus: $<HTMLDivElement>('#connection-status'),
    toggleLocalMode: $<HTMLInputElement>('#toggle-local-mode'),
    localModeStatus: $<HTMLSpanElement>('#local-mode-status'),
    inputEndpoint: $<HTMLInputElement>('#input-endpoint'),
    inputApiKey: $<HTMLInputElement>('#input-api-key'),
    inputModel: $<HTMLInputElement>('#input-model'),
    // Ghost Persona
    inputStyleSample: $<HTMLTextAreaElement>('#input-style-sample'),
    btnAnalyzeStyle: $<HTMLButtonElement>('#btn-analyze-style'),
    styleProfileDisplay: $<HTMLDivElement>('#style-profile-display'),
    styleTone: $<HTMLSpanElement>('#style-tone'),
    styleLength: $<HTMLSpanElement>('#style-length'),
    styleTraits: $<HTMLDivElement>('#style-traits'),
    // Loading
  ...
  function applySettingsToUI() {
    if (els.inputEndpoint) els.inputEndpoint.value = settings.cloudEndpoint;
    if (els.inputApiKey) els.inputApiKey.value = settings.cloudApiKey;
    if (els.inputModel) els.inputModel.value = settings.cloudModel;
    if (els.toggleLocalMode) els.toggleLocalMode.checked = settings.localModeEnabled;
  }
  ...
  async function checkLocalModelAvailability() {
    ...
    if (!els.localModeStatus || !els.toggleLocalMode) return;
  ...
  async function saveSettings() {
    if (!els.inputEndpoint || !els.inputApiKey || !els.inputModel || !els.toggleLocalMode) return;
  ...
  function precheckExecution(): { valid: boolean; error?: string } {
  ...
  async function handleExplain() {
    ...
    if (!text || !els.searchResultText || !els.searchResult) return;
    ...
    try {
      ...
      const result = await AIServiceFactory.callAI(
        systemPrompt,
        `Please explain this:\n\n"${text}"`,
        settings,
        styleProfile
  ...
  async function handleRefine() {
    ...
    if (!rawPrompt || !els.sandboxOutput) return;
  ...
  async function handleDraft() {
    ...
    if (!els.emailResultText || !els.emailResult) return;
...
- Do NOT include a subject line — just the reply body`;
  ...
  function displayStyleProfile() {
    if (!els.styleTone || !els.styleLength || !els.styleTraits || !els.styleProfileDisplay) return;
  ...
  function activateTab(tabName: string) {
    els.tabBtns.forEach((b) => b.classList.remove('active'));
    els.tabPanels.forEach((p) => p.classList.remove('active'));
    const btn = $<HTMLButtonElement>(`.tab-btn[data-tab="${tabName}"]`);
    if (btn) btn.classList.add('active');
    const panel = $<HTMLElement>(`#tab-${tabName}`);
    if (panel) panel.classList.add('active');
  }

  function handleSelectionSearchMessage(text: string) {
    activateTab('search');
    if (els.searchInput) {
  ...
  function handleEmailDraftMessage(threadText: string) {
    activateTab('email');
    if (els.emailThread) {
  ...
  function showCopyTooltip(btn: HTMLElement) {
